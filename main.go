...
func main() {
	...
	provider := flags.String("provider", "openai", "provider to use: openai or gemini")
	...
	flags.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: auth-rotate [rotate|sync] [options]\n\n")
		fmt.Fprintf(os.Stderr, "Rotate or sync OAuth accounts from a central config.\n\n")
	...
	default:
		...
		fmt.Fprintf(os.Stderr, "unknown command: %s (use rotate or sync)\n", command)
		os.Exit(1)
	}
}
